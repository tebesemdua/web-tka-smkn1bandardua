...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  const navItems = [
    ...
    { id: 'absensi-siswa', label: 'Data & Absensi Siswa', icon: UserCheck, roles: ['admin', 'guru'] },
    { id: 'absensi-guru', label: 'Master Guru & Absensi', icon: Briefcase, roles: ['admin', 'guru'] },
    { id: 'rekap-nilai', label: 'Rekap Nilai CBT', icon: FileSpreadsheet, roles: ['admin', 'guru'] },
    { id: 'download-hub', label: 'Download Hub', icon: Download, roles: ['admin', 'guru'], badge: 'Admin' },
    { id: 'grafik-analytics', label: 'Grafik & Status Real', icon: BarChart3, roles: ['admin', 'guru'] },
  ];

  const visibleNavItems = navItems.filter(item => item.roles.includes(currentUser.role));

  const roleColors: Record<UserRole, { bg: string; text: string; label: string }> = {
    admin: { bg: 'bg-red-500/15 border-red-500/30 text-red-700', text: 'text-red-700', label: '🛡️ Admin' },
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-40 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between min-h-[5.5rem] py-2">
            ...
            {/* Right: Clock & User Profile Bar with Initials Avatar + Name & Email Directly Below Avatar */}
            <div className="hidden lg:flex items-center gap-5">
              ...
              {currentUser.role === 'guest' ? (
                ...
                <div className="flex items-center gap-3 pl-1">
                  {/* Tampilan Profil: Ikon Inisial di atas, Nama Asli & Email TEPAT DI BAWAHNYA */}
                  <div className="flex flex-col items-center justify-center text-center bg-slate-50/90 border border-slate-200/90 rounded-2xl px-3 py-1.5 shadow-xs min-w-[160px] max-w-[210px]">
                    {/* Ikon Profil Inisial */}
                    ...
                    {/* Tepat di bawah ikon: Nama Asli & Email */}
                    <div className="mt-1 w-full">
                      <div className="text-xs font-extrabold text-slate-900 truncate leading-tight" title={currentUser.name}>
                        {currentUser.name}
                      </div>
                      <div className="text-[10px] text-emerald-700 font-semibold truncate leading-tight mt-0.5" title={currentUser.email}>
                        {currentUser.email}
                      </div>
                      <div className="mt-1 flex items-center justify-center">
                        <span className={`text-[9px] font-extrabold px-1.5 py-0.2 rounded-full border ${roleColors[currentUser.role].bg}`}>
                  ...
                  <div className="flex flex-col gap-1.5">
                    {/* Button Edit Profil untuk Guru */}
                    {currentUser.role === 'guru' && onOpenEditProfile && (
                      <button
                        onClick={onOpenEditProfile}
                        title="Edit Profil Guru Saya"
                        className="px-3 py-1 bg-emerald-50 hover:bg-emerald-100 text-emerald-800 rounded-xl transition-colors cursor-pointer border border-emerald-300 flex items-center justify-center gap-1 text-[11px] font-bold shadow-xs"
                      >
                        <UserCog className="w-3.5 h-3.5 text-emerald-700" />
                    ...
                    <button
                      ...
                      className="px-3 py-1.5 text-red-600 hover:text-white hover:bg-red-600 rounded-xl transition-colors cursor-pointer border border-red-200 flex items-center justify-center gap-1.5 text-xs font-bold shadow-xs"
                    >
            ...
            <div className="flex items-center gap-2 lg:hidden">
              {currentUser.role === 'guest' ? (
                ...
                <div className="flex items-center gap-1.5">
                  <UserAvatar name={currentUser.name} role={currentUser.role} size="xs" />
                  <button
                    onClick={onLogout}
                    title="Keluar"
        ...
        {mobileMenuOpen && (
          <div className="lg:hidden bg-white border-b border-slate-200 px-4 pt-3 pb-5 space-y-3 shadow-xl animate-fadeIn">
            {currentUser.role !== 'guest' ? (
              <div className="p-3 bg-slate-50 rounded-2xl border border-slate-200 flex flex-col items-center text-center space-y-2">
                {/* Mobile Avatar Inisial & Nama/Email Tepat di Bawahnya */}
                <UserAvatar 
                  name={currentUser.name} 
                  role={currentUser.role} 
                  size="md" 
                ...
                <div>
                  ...
                  <div className="mt-1">
                    <span className={`text-[9px] font-bold px-2 py-0.5 rounded-full border ${roleColors[currentUser.role].bg}`}>
                      {roleColors[currentUser.role].label}
                    </span>
                  </div>
                </div>
                ...
                {currentUser.role === 'guru' && onOpenEditProfile && (
                  <button
                    ...
                    className="w-full py-1.5 text-xs font-bold bg-emerald-50 text-emerald-800 border border-emerald-300 rounded-xl flex items-center justify-center gap-1.5"
                  >
                    <UserCog className="w-3.5 h-3.5" />
                    <span>Edit Profil Guru Saya</span>
                  </button>
                )}
              </div>
            ) : (
              <div className="p-3 bg-emerald-50/70 rounded-xl flex items-center justify-between border border-emerald-200">
                <div>
